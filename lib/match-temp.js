...
Match.prototype.search = function (options, documents, subSet) {
	...
	if(options.query){
		items = items.filter(function(item){
			return self._match(item, options.query); 
		})
	 ...
	 * @private
	 */
Match.prototype._match = function (source, test, opToApply, options) {
	 ...
	 * @private
	 */
Match.prototype._matchOp = function (key, source, test, options) {
		// Check for commands
		switch (key) {
			case '$gt':
				// Greater than
